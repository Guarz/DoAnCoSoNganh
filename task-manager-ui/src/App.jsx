...
function App() {
  ...
  const showDetail = async (orderId) => {
    try {
      const res = await axios.get(
        `http://127.0.0.1:8000/api/get-order-details/${orderId}`
      );
  ...
  const renderContent = () => {
    ...
    switch (activeMenu) {
          ...
          <div>
            ...
            >
              <div style={cardStyle("#3498db")}>
                <h1>{data.stats.total_products || 0}</h1>
                <p>Sản phẩm</p>
              </div>
              <div style={cardStyle("#27ae60")}>
                <h1>{data.stats.total_categories || 0}</h1>
                <p>Danh mục</p>
              </div>
              <div style={cardStyle("#f39c12")}>
                <h1>{data.stats.total_orders || 0}</h1>
                <p>Đơn hàng</p>
              </div>
              <div style={cardStyle("#e74c3c")}>
                <h1>{data.stats.total_users || 0}</h1>
                <p>Thành viên</p>
              </div>
            </div>
          </div>
      ...
      case "categories":
        return renderTable(["ID", "Tên danh mục"], data.categories, [
          "category_id",
          "category_name",
        ]);
      ...
      case "orders":
        return (
          <div>
            ...
            {renderTable(
              ["ID", "Khách hàng", "Tổng tiền", "Trạng thái", "Thao tác"],
              data.orders,
      ...
      case "users":
        return renderTable(["ID", "Tên", "Email", "Ngày tạo"], data.users, [
          "user_id",
          "name",
          "email",
          "created_at",
      ...
      default:
        return <h3>Chức năng đang cập nhật...</h3>;
    }
  };
                ...
                return (
                  <td key={k} style={tdStyle}>
                    <select
                      value={r[k]}
                      onChange={(e) =>
                        updateOrderStatus(r.order_id, e.target.value)
                      }
    ...
    <div style={{ display: "flex", minHeight: "100vh", width: "100vw" }}>
      ...
      >
        ...
        <nav style={{ padding: "15px" }}>
          {["dashboard", "products", "categories", "orders", "users"].map(
            (m) => (
              <div
                key={m}
                onClick={() => {
...
const navItem = (a) => ({
  ...
  borderRadius: "8px",
  marginBottom: "5px",
  backgroundColor: a ? "#3498db" : "transparent",
  color: a ? "white" : "#bdc3c7",
});
const cardStyle = (c) => ({
  padding: "30px",
  backgroundColor: c,

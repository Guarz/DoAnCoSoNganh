    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const res = await fetch(
          `http://127.0.0.1:8000/api/admin/dashboard?t=${Date.now()}`
        );
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ marginTop: "30px", display: "flex", gap: "15px" }}>
        <Link to="/admin/products" style={btnStyle}>
          Quản lý sản phẩm
        </Link>
        <Link to="/admin/categories" style={btnStyle}>
          Quản lý danh mục
        </Link>
...
const btnStyle = {
  ...
  borderRadius: "10px",
  fontWeight: "bold",
  boxShadow: "0 4px 10px rgba(255, 77, 141, 0.3)",
};
